...
function loadSequelizeConfig() {
  ...
  const configPath = path.isAbsolute(seqmigConfig.configFile) ? seqmigConfig.configFile : path.join(process.cwd(), seqmigConfig.configFile);
...
async function introspect() {
  ...
  sequelize.import = (filePath) => {
    if (!filePath.endsWith(".ts") && !filePath.endsWith(".js")) {
      if (fs2.existsSync(filePath + ".ts")) filePath = filePath + ".ts";
      if (fs2.existsSync(filePath + ".js")) filePath = filePath + ".js";
    }
    return import(filePath);
  };
  const modelsPath = path2.isAbsolute(seqmigConfig.modelsPath) ? seqmigConfig.modelsPath : path2.join(process.cwd(), seqmigConfig.modelsPath);
...
function generate(actions) {
  ...
  const dir = path3.join(process.cwd(), "src/migrations");
  if (!fs3.existsSync(dir)) fs3.mkdirSync(dir, {
    recursive: true
  });
  const createTables = [];
...
function getSnapshotPaths() {
  ...
  const SNAPSHOT_FILE = path4.join(process.cwd(), snapshotDir, "schema-snapshot.json");
  const SNAPSHOT_BACKUP_DIR = path4.join(process.cwd(), snapshotDir, "backups");
  return {
    SNAPSHOT_FILE,
    SNAPSHOT_BACKUP_DIR,
    migrationDir,
...
function loadSnapshot() {
  ...
  const raw = fs4.readFileSync(SNAPSHOT_FILE, "utf8");
  return JSON.parse(raw);
}
__name(loadSnapshot, "loadSnapshot");
function saveSnapshot(schema) {
  const { SNAPSHOT_FILE, SNAPSHOT_BACKUP_DIR } = getSnapshotPaths();
  const dir = path4.dirname(SNAPSHOT_FILE);
  if (!fs4.existsSync(dir)) fs4.mkdirSync(dir, {
    recursive: true
  });
  if (!fs4.existsSync(SNAPSHOT_BACKUP_DIR)) fs4.mkdirSync(SNAPSHOT_BACKUP_DIR, {
    recursive: true
  });
  if (fs4.existsSync(SNAPSHOT_FILE)) {
    const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
    const backupFile = path4.join(SNAPSHOT_BACKUP_DIR, `snapshot-${timestamp}.json`);
    fs4.copyFileSync(SNAPSHOT_FILE, backupFile);
    cleanOldBackups(SNAPSHOT_BACKUP_DIR, 20);
  }
  fs4.writeFileSync(SNAPSHOT_FILE, JSON.stringify(schema, null, 2));
}
__name(saveSnapshot, "saveSnapshot");
function cleanOldBackups(backupDir, keepCount) {
  const backups = fs4.readdirSync(backupDir).filter((f) => f.startsWith("snapshot-")).sort().reverse();
  backups.slice(keepCount).forEach((file) => {
    fs4.unlinkSync(path4.join(backupDir, file));
  });
}
__name(cleanOldBackups, "cleanOldBackups");
function listBackups() {
  const { SNAPSHOT_BACKUP_DIR } = getSnapshotPaths();
  if (!fs4.existsSync(SNAPSHOT_BACKUP_DIR)) return [];
  return fs4.readdirSync(SNAPSHOT_BACKUP_DIR).filter((f) => f.startsWith("snapshot-")).sort().reverse();
}
__name(listBackups, "listBackups");
function restoreBackup(backupFile) {
  const { SNAPSHOT_FILE, SNAPSHOT_BACKUP_DIR } = getSnapshotPaths();
  const backupPath = path4.join(SNAPSHOT_BACKUP_DIR, backupFile);
  if (!fs4.existsSync(backupPath)) {
    throw new Error(`Backup file not found: ${backupFile}`);
  }
  fs4.copyFileSync(backupPath, SNAPSHOT_FILE);
...
function hasSequelizeCli() {
  ...
  const cliPath1 = path5.join(cwd, "node_modules/.bin/sequelize");
  const cliPath2 = path5.join(cwd, "node_modules/.bin/sequelize-cli");
  return fs5.existsSync(cliPath1) || fs5.existsSync(cliPath2);
}
